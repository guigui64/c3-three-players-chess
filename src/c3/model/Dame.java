...
public class Dame extends Piece {
    ...
    public Dame(Couleur couleur, Joueur joueur) {
    ...
    public void detecterCasesDisponibles(Case caseDepart) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            while (caseEnCours != caseDispDE(caseEnCours, directions[i], false) && casesAccessiblesPourPrise.contains(caseEnCours) == false) {// tant que la prochaine case dans la même direction est différente de la case en cours et qu'on a pas une pièce de couleur adverse dessus, on continue dans cette direction
                if (caseEnCours.getLigne() == 4 && caseEnCours.getColonne() == i && (i == 4 || i == 5)) // directions supplementaires du fou
                {
                    directionSupplementaire = true;
                }
                if (caseEnCours.getLigne() == 4 && caseEnCours.getColonne() == 4 && (i == 0 || i == 2)) // directions supplementaires de la tour
                {
                    directionSupplementaire = true;
                }
                ...
                if (caseEnCours.getZone() != caseDispDE(caseEnCours, directions[i], false).getZone()) // on doit faire le test avant de changer la case en cours
                {
                    ...
                    switch (i) {
                        case 0:
                            ...
                            break;
                        case 4:
                            directions[4] = directions[7];
                            break;
                        case 5:
                            directions[5] = directions[6];
                            break;
            ...
            if (directionSupplementaire == true) {
                ...
                if (i == 4 || i == 5) {
                    ...
                    caseEnCours = echiquier.getCases().get(echiquier.getIndexCase(4, i, caseDepart.getZone())); // on part de la case critique
                    if (caseEnCours != caseDispDESupplementaire(caseEnCours, directions[i], false, false)) { // on teste si la case dans la direction supplémentaire n'est pas occupée par une de nos pièces
                        ...
                        if (caseEnCours.getPiece() != null) // s'il y a une pièce adverse dessus, la recherche s'arrête
                        {
                            ...
                            if (i == 4) {
                                directions[i] = Direction.BASDROITE;
                            } else {
                                directions[i] = Direction.BASGAUCHE; // on change la direction parce qu'on a  à nouveau changé de zone
                            }
                            while (caseEnCours != caseDispDE(caseEnCours, directions[i], false) && casesAccessiblesPourPrise.contains(caseEnCours) == false) {
                                caseEnCours = caseDispDE(caseEnCours, directions[i], false);
                                if (caseEnCours.getPiece() == null) {
